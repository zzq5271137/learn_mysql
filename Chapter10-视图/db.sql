...
-- ----------------------------
-- View structure for emp_dep_sal_avg_view
-- ----------------------------
DROP VIEW IF EXISTS `emp_dep_sal_avg_view`;
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`localhost` SQL SECURITY DEFINER VIEW `emp_dep_sal_avg_view` AS select `emp`.`deptno` AS `deptno`,avg(`emp`.`salary`) AS `avg(salary)` from `emp` group by `emp`.`deptno`;

-- ----------------------------
-- View structure for emp_sal_view
-- ----------------------------
DROP VIEW IF EXISTS `emp_sal_view`;
CREATE ALGORITHM = MERGE DEFINER = `root`@`localhost` SQL SECURITY DEFINER VIEW `emp_sal_view` AS select `emp`.`empno` AS `empno`,`emp`.`ename` AS `ename`,`emp`.`salary` AS `salary` from `emp` where (`emp`.`salary` > 2000)  WITH CASCADED CHECK OPTION;
